...
export function DevelopmentPage() {
  ...
  return (
    <AppShell eyebrow="System administration • Interactive demo" title="Development dashboard" subtitle="Store rollout, feature access, and usage activity" actions={<Link className="button ghost small" to="/store/CFA00000">Store demo</Link>}>
      ...
      <div className="dashboard-toolbar"><div className="segmented"><button className={tab === "active" ? "active" : ""} type="button" onClick={() => setTab("active")}>Active ({activeCount})</button><button className={tab === "blocked" ? "active" : ""} data-tour-id="blocked-tab" type="button" onClick={() => setTab("blocked")}>Paywall blocked ({stores.length - activeCount})</button></div><span className="sync-label">Simulated usage data</span></div>
      {visible.length ? <section className="store-admin-list" data-tour-id="development-controls">{visible.map((store) => (
        <article className="store-admin-card" key={store.storeCode}>
          ...
          <NicknameEditor store={store} tourTarget={store.storeCode === "CFA00000"} save={(nickname) => mutate(() => saveConfig(store.storeCode, { ...store.config, nickname: nickname.trim() || undefined }), `${store.storeCode} nickname updated.`)} onSaved={() => completeStep("nickname-form")} />
          <div className="feature-controls">
            ...
            <label><input type="checkbox" data-tour-id={store.storeCode === "CFA00000" ? "development-email-toggle" : undefined} checked={store.config.featureFlags.emailAlertsLive} onChange={(event) => mutate(() => saveConfig(store.storeCode, { ...store.config, featureFlags: { ...store.config.featureFlags, emailAlertsLive: event.target.checked } }), `${store.storeCode} email access updated.`)} /> Email alerts live</label>
...
function NicknameEditor({ store, save, onSaved, tourTarget = false }: { store: DemoStore; save: (nickname: string) => Promise<void>; onSaved?: () => void; tourTarget?: boolean }) {
  const saved = store.config.nickname || "";
  const [nickname, setNickname] = useState(saved);
  const [busy, setBusy] = useState(false);
  useEffect(() => setNickname(saved), [saved]);
  return <form className="store-nickname-form" data-tour-id={tourTarget ? "nickname-form" : undefined} onSubmit={async (event) => { event.preventDefault(); setBusy(true); try { await save(nickname); onSaved?.(); } finally { setBusy(false); } }}><label>Store nickname<input value={nickname} onChange={(event) => setNickname(event.target.value)} maxLength={80} placeholder="Example: Downtown" /></label><button className="button primary small" disabled={busy || nickname.trim() === saved}>{busy ? "Saving…" : "Save nickname"}</button></form>;
}
