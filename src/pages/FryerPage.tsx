...
export function FryerPage() {
  ...
  return (
    <div className="page-shell">
      <header className="topbar"><div className="topbar-left"><button className="back" onClick={() => navigate(`/store/${storeCode}`)} aria-label="Back">←</button><h1>{fryer.name}</h1></div><Link data-tour-id="dashboard-link" to={`/store/${storeCode}`} onClick={() => completeStep("dashboard-link")}>Dashboard</Link></header>
      ...
      <main className="wrap fryer-wrap">
        ...
        <section className="actions"><button className="action-btn primary" data-tour-id="log-button" type="button" onClick={() => { setDialog("log"); completeStep("log-button"); }}><span>Log Boil Out</span><span>→</span></button><button className="action-btn secondary" data-tour-id="flag-button" type="button" onClick={() => { setDialog("flag"); completeStep("flag-button"); }}><span>Boil Out Needed</span><span>!</span></button></section>

        <section className="dropdown-section" data-tour-id="history-section">
          <button className={`dropdown-toggle ${historyOpen ? "open" : ""}`} data-tour-id="history-toggle" type="button" onClick={() => { setHistoryOpen(!historyOpen); completeStep("history-toggle"); }}><span className="dropdown-toggle-title">Boil Out History</span><span className="dropdown-toggle-icon">⌄</span></button>
        ...
        <section className="dropdown-section" data-tour-id="leader-tools">
          <button className={`dropdown-toggle ${leaderOpen ? "open" : ""}`} data-tour-id="leader-toggle" type="button" onClick={() => { leaderUnlocked ? setLeaderOpen(!leaderOpen) : setDialog("leader"); completeStep("leader-toggle"); }}><span className="dropdown-toggle-title">Leader Tools</span><span className="dropdown-toggle-icon">⌄</span></button>
